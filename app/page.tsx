    ...
    <main className="min-h-screen bg-white text-neutral-900">
      ...
      <section className="mx-auto max-w-3xl px-4 py-16">
        ...
        <div className="mt-6 divide-y divide-neutral-200 rounded-2xl border border-neutral-200 bg-white">
          {[
            { q: "Do I need Spotify Premium?", a: "No, Song Starter uses public artist data from Spotify's Web API and ReccoBeats' API." },
            { q: "Do I need a Suno account?", a: "You can use Suno's free tier to generate music with these prompts." },
            ...
            <details key={i} className="group px-5 py-4">
              <summary className="flex cursor-pointer list-none items-center justify-between text-sm font-medium">
                <span>{item.q}</span>
                <ChevronDown className="w-4 h-4 text-neutral-500 transition group-open:rotate-180" />
              </summary>
      ...
      <footer className="border-t border-neutral-200 py-8">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-4 text-sm text-neutral-600 md:flex-row">
          <div>© {new Date().getFullYear()} Song Starter</div>
          <div className="text-xs">Not affiliated with Spotify or Suno.</div>
        </div>
      </footer>
    </main>
  )
